...
class GenerateGraph:
    ...
    def generateGraphForActualAndPredictedDengueCases(self, startDate, endDate, predCases, displayWeather):
        ...
        p = figure(plot_width=1000, plot_height=400, title="Actual Cases vs Predicted Cases",
                   x_range=histCases['date_str'])
        ...
        if (displayWeather):
            ...
            p1 = p.vbar(x=dodge('date_str', -0.25, range=p.x_range), top='mean_temp', width=0.1, source=source,
                        color='#3288bd')
            p2 = p.vbar(x=dodge('date_str', -0.15, range=p.x_range), top='max_temp', width=0.1, source=source,
                        color='#99d594')
            p3 = p.vbar(x=dodge('date_str', -0.05, range=p.x_range), top='min_temp', width=0.1, source=source,
                        color='#e6f598')
            p4 = p.vbar(x=dodge('date_str', 0.05, range=p.x_range), top='humidity', width=0.1, source=source,
                        color='#fee08b')
            p5 = p.vbar(x=dodge('date_str', 0.15, range=p.x_range), top='mean_rain_fall', width=0.1, source=source,
                        color='#fc8d59')
            p6 = p.vbar(x=dodge('date_str', 0.25, range=p.x_range), top='mean_wind', width=0.1, source=source,
                        color='#d53e4f')
